...
bool __is_page_table_empty(uint32* ptr_page_table);

void __free_buffered_pages(struct Env* e, uint32 va, uint32 number_of_pages);
void __free_working_set_pages(struct Env* e, uint32 va, uint32 number_of_pages);
void __free_empty_page_tables(struct Env* e, uint32 va, uint32 number_of_pages);
void __free_pages_from_page_file(struct Env* e, uint32 va, uint32 number_of_pages);
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	__free_buffered_pages(e, virtual_address, number_of_pages);

	__free_working_set_pages(e, virtual_address, number_of_pages);

	__free_empty_page_tables(e, virtual_address, number_of_pages);

	__free_pages_from_page_file(e, virtual_address, number_of_pages);
}

void __free_buffered_pages(struct Env* e, uint32 va, uint32 number_of_pages) {
	for (uint32 i = 0; i < number_of_pages; i++, va += PAGE_SIZE) {
...
void __free_working_set_pages(struct Env* e, uint32 va, uint32 number_of_pages) {
	for (uint32 i = 0; i < e->page_WS_max_size; i++) {
	 	uint32 ws_virtual_address = e->ptr_pageWorkingSet[i].virtual_address;
		if (ws_virtual_address >= va && ws_virtual_address < va + number_of_pages * PAGE_SIZE) {
			unmap_frame(curenv->env_page_directory, (void*)ws_virtual_address);
...
void __free_empty_page_tables(struct Env* e, uint32 va, uint32 number_of_pages) {
	for (uint32 i = 0; i < number_of_pages; i++, va += PAGE_SIZE) {
		...
		if (__is_page_table_empty(ptr_page_table)) {
...
bool __is_page_table_empty(uint32* ptr_page_table) {
	uint32 entries_per_table = PAGE_SIZE / 4; 	// since uint32 size is 4 bytes.

	for (uint32 i = 0; i < entries_per_table; i++)
		if(ptr_page_table[i] != 0)
			return 0; // entry found; page table is not empty.
